...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// DSH renamed this module id in 0.1.2-alpha: `@deepseek-ai/dsh-client-runtime`
		// is gone and its seed row is now the bare `@deepseek-ai/dsh-client-store`.
		// An unknown id throws straight out of the module table, so ask for the
		// current seed word first and keep the old one as a fallback.
		let _client_store;
		try {
			_client_store = require("@deepseek-ai/dsh-client-store");
		} catch {
			_client_store = require("@deepseek-ai/dsh-client-runtime/client");
		}
